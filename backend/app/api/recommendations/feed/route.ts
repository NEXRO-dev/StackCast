...
import { dailyEditionForUser, tokyoEditionDate } from "@/lib/recommendations/daily-edition";
...
export async function GET(request: Request) {
  ...
  const editionDate = requestedDate && /^\d{4}-\d{2}-\d{2}$/.test(requestedDate) ? requestedDate : tokyoEditionDate();
  try {
