...
export function PublicCastPage({ cast, token, language }: { cast: PublicCast; token: string; language: "ja" | "en" }) {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#ede9fe_0,#f8fafc_42%,#fff_100%)] px-5 py-12 text-slate-950 sm:py-20">
      <section className="mx-auto flex max-w-lg flex-col items-center rounded-[2rem] border border-white/80 bg-white/75 p-6 shadow-[0_24px_80px_rgba(76,29,149,0.14)] backdrop-blur-xl sm:p-10">
        ...
        <p className="mb-2 text-sm font-semibold tracking-wide text-indigo-600">StackCast</p>
        <h1 className="text-center text-2xl font-bold leading-tight tracking-tight sm:text-3xl">{cast.title}</h1>
        <p className="mt-3 text-sm font-medium text-slate-500">{isJapanese ? `${cast.durationMinutes}分のCast` : `${cast.durationMinutes}-minute Cast`}</p>
        ...
        <audio className="mt-8 w-full" controls preload="metadata" src={`/api/public/casts/${token}/audio`}>
          {isJapanese ? "お使いのブラウザは音声再生に対応していません。" : "Your browser does not support audio playback."}
        </audio>
        <a href={`stashcast://cast/${token}`} className="mt-8 inline-flex min-h-12 w-full items-center justify-center rounded-full bg-slate-950 px-6 font-semibold text-white transition hover:bg-indigo-700">{isJapanese ? "StackCastで開く" : "Open in StackCast"}</a>
        <p className="mt-5 text-center text-xs leading-5 text-slate-400">{isJapanese ? "共有されたCastだけを再生します。文字起こしや作成者の個人情報は公開されません。" : "Only the shared Cast is played. Transcripts and the creator’s personal information are not published."}</p>
      </section>
    </main>
  );
}
