...
`vercel.json` は5分間隔で `GET /api/internal/news/daily` を呼び出し、各ユーザーの保存タイムゾーンで16:15〜16:29を日次処理の実行・再試行枠として扱います。この定期処理では直近取得のcooldownを無視し、既に当日の5件がある場合も版を作り直します。Providerが利用できない場合も30日間の共通キャッシュからfallback版を作ります。Vercel側には十分に長い `CRON_SECRET` を設定してください。候補ニュースはGDELTからジャンル単位で共通取得し、ユーザーごとの興味・閲覧メモリから5件のデイリー版を固定します。
